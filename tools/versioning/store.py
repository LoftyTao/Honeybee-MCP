...
def list_versions(model_name: str) -> dict:
    if model_name not in _version_history:
        return {"success": False, "error": "No versions found for model '{}'".format(model_name)}
...
def load_version(model_name: str, version_id: str) -> dict:
    ...
    return {"success": False, "error": "Version {} not found".format(version_id), "available_versions": available}
...
def redo_last(model_name: str) -> dict:
    if model_name not in _redo_stack or not _redo_stack[model_name]:
        return {"success": False, "error": "No redo available. Nothing has been undone."}
    ...
    return {"success": False, "error": "Version {} not found in history".format(version_id)}
...
def compare_versions(model_name: str, version_id_1: str, version_id_2: str) -> dict:
    ...
    if v1 is None:
        return {"success": False, "error": "Version {} not found".format(version_id_1)}
    if v2 is None:
        return {"success": False, "error": "Version {} not found".format(version_id_2)}
...
def clear_versions(model_name: str = None) -> dict:
    ...
    if model_name:
        if model_name in _version_history:
            ...
            return {"success": True, "message": "Cleared all versions for model '{}'".format(model_name)}
        return {"success": False, "error": "Model '{}' not found".format(model_name)}

    count = len(_version_history)
    _version_history = {}
    _version_counter = {}
    _redo_stack = {}
    return {"success": True, "message": "Cleared all versions for {} models".format(count)}
